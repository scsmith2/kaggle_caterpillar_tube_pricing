...
rm(BOM, adapt, boss, comps, elbow, float, hfl)
rm(nut, other, out, sleeve, spec, straight, suppliers, tee, thread)
rm(tube, tubeEnd, typeComp, typeCon, typeEnd, allSpec, asmID, bracketLevs)
rm(compID, comp_vars, datadir, i, j, pi, sigSpecs, sigSpecsDash)
rm(tubeEnds, tubeMatls, tube_Matls)

mod1 <- xgb.train(params = params, data = dtrain, nrounds = 4000, verbose = 1)
preds1 <- predict(mod1,dtest)

mod2 <- xgboost(params = params, data = dtrain, nrounds = 4500, verbose = 1)
preds2 <- predict(mod2,dtest)

mod3 <- xgboost(params = params, data = dtrain, nrounds = 5000, verbose = 1)
...
if (submit==TRUE){
  ...
  write.csv(out, file = "cat_xgb_3.csv", quote = F, row.names = F)
}